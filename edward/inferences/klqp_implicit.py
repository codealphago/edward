...
def klqp_implicit(model, variational, discriminator, align_latent,
                  align_latent_global, align_data, ratio_log='log',
  ...
  for name, node in six.iteritems(model_trace):
    ...
    posterior_node = posterior_trace.get(align_latent_global(name), None)
    if posterior_node is not None:
      pbeta = node.value
      qbeta = posterior_node.value
      ...
      qbeta_sample[pbeta] = qbeta.value
    else:
